# Workspace Document Map

This file lists all documents and major files in the workspace, organized by folder.

## Root
- README.md
- MyVaronis-BigResultHandler.sln
- git-push.bat

## CV
- ArielCV.md
- Ariel_CV_Original.pdf
- Arithmetic_Interview_Questions.md
- CV guidelines by Claude.pdf
- CV guidelines by Claude.txt
- CV_FEEDBACK.md
- HR_Questions.md
- Logic_Programming_Interview_Questions.md

 **NEXT_WORK_ITEMS.md**: Tracks upcoming tasks, TODOs, and planned work for the project. Update regularly to keep the team aligned.

## docs
- architecture-diagram.html
- ARCHITECTURE.md
- architecture.mermaid
- Catching up with AI IDE stub
- How ResultHandler Improved The Old Solution.md
- RabbitMQ Fundamentals Guide.md
- CRASH_RECOVERY.md
- CRITICAL_ARCHITECTURE_QUESTIONS.md: Resolved/unresolved architecture questions including 6 scalability obstacles and design trade-offs
- CSharp_Basics.md
- General Architecture questions.md
- INTERVIEW_QUESTIONS.html
- INTERVIEW_QUESTIONS.md
- INTERVIEW_QUESTIONS.txt
- interview_questions_newInfo.txt
- RESULT_ASSEMBLER_LLD.md
- RESULT_HANDLER_LLD.md
- STATE_MACHINE_LLD.md
- TPLvsRX/
- Varonis projects
- concurrent data structures.md
- buzzwords_and_services.md

## Positions
- Preparation for Commbox.md
- Preparation for Mastercard.md
- Preparation For Semperis.md
- Preparation for mPrest

### docs/TPLvsRX
- Reactive vs TPL handling in RabbitMQ messages.md
- ResultHandlerWithObservable.cs
- ResultHandlerWithTPL.cs
- RxSubjectProducerSubscriberExample.cs
- RxSubjectRabbitMqExample.cs

## Playground
- Playground.csproj
- Program.cs

## Positions
- Preparation For Semperis.md
