...
public class RabbitMessageHandler
{
    ...
    {
        _useSubjectProcessor = useSubjectProcessor;

        if (useSubjectProcessor)
            _subjectProcessor = new SubjectMessageProcessor(); 
        else 
            _dataflowProcessor = new DataflowMessageProcessor();
    }

    public void OnMessageReceived(IMessage msg)
    {
        Console.WriteLine($"[RabbitMessageHandler] Received: {msg.Body}");
        if (_useSubjectProcessor)
            _subjectProcessor.Process(msg);
        else
            _dataflowProcessor.Process(msg);
    }
}

// Simulated RabbitMQ message interface
...
public class RabbitMessage : IMessage
{
    ...
    public RabbitMessage(string body) => Body = body;
}
...
class Program
{
    ...
    {
        bool useSubjectProcessor = bool.Parse(args.Length > 0 ? args[0] : "false");
        
        var handler = new RabbitMessageHandler(useSubjectProcessor);
        Task.Run(() =>
        {
            for (int i = 1; i <= 5; i++)
            {
                Thread.Sleep(500); // Simulate message arrival delay
                var msg = new RabbitMessage($"RabbitMQ Message {i}");
                handler.OnMessageReceived(msg);
            }
            handler.Complete();
        });

        Thread.Sleep(4000); // Keep main thread alive to see all output
    }
}
